...
class tb_municipio(db.Model):
    ...
    codmunicipio: Mapped[int] = mapped_column('codmunicipio', Integer, primary_key=True)
    ...
    instituicoes: Mapped[List[tb_instituicao]] = relationship(
        ...
        primaryjoin="tb_municipio.codmunicipio == tb_instituicao.codmunicipio"
    )

    def __init__(self, codmunicipio: int, nome_municipio: str):
        self.codmunicipio = codmunicipio
        self.nome_municipio = nome_municipio

    def __repr__(self):
        return f"tb_municipio(id={self.codmunicipio}, nome={self.nome_municipio})"